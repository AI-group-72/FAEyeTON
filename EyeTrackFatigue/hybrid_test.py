...
import pandas as pd
from Evaluate.HybridEval import HybridEval
...
# вызов подготовки данных 
# data_preprocessing('D:\\PythonProjects\\Faeyeton\\_Data//all_au.csv')
# загрузка данных
...
'''
'''
data_list = pd.read_csv('D:\\PythonProjects\\Faeyeton\\_Data//all_au.csv', delimiter=';')
data_list.drop(columns=['File', 'AU_cat'], inplace=True) #, 'fatigue', 'Concen', 'Имя файла', 'Метод определения', 'деятельность', 'время суток'], inplace=True)
data_list.dropna()
#data_list.drop(columns=['duration_blink_mean', 'duration_blink_min', 'duration_blink_max', 'duration_fixation_mean', 'duration_fixation_min', 'duration_fixation_max', 'x_fixation_mean', 'x_fixation_min'], inplace=True)
#data_list.drop(columns=['x_fixation_max', 'y_fixation_mean', 'y_fixation_min', 'y_fixation_max', 'azimuth_fixation_mean', 'azimuth_fixation_min', 'azimuth_fixation_max', 'elevation_fixation_max', 'elevation_fixation_min', 'elevation_fixation_mean'], inplace=True)
data = pd.read_csv('m1.csv', sep=';')
data_files = data.pop('File')
