function test_metrics_fixsations(S, mode)
...
featurePath = strcat('D:\Projects\USC#', S, '_encoded_0000.mp4+fixsac_features.csv');
ReultsPathCSV = strcat('D:\Projects\USC#', S, '-correlation.csv');

RR = readRR(rrPath);
[allFixTime, allSaccadeLen] = readMV(featurePath);
...
if mode == 1
    writeLine(S, 'D:\Projects\Results\metrics-Fix_Time.csv');
else
    writeLine(S, 'D:\Projects\Results\metrics-Fix_Len.csv');
end
